...
def export_to_onnx(model_path, model_id, use_quantization=False):
    ...
    # Optimize by reducing precision (32bit float to 8bit int) to reduce model size and improve inference speed
    if use_quantization:
    ...
    dummy_input = torch.randn(1, config.config["input_channels"], 240, 520)

    onnx_model_path = config.config["save_model_path"] + "/onnx_models/model_" + str(model_id) + ".onnx"
...
if __name__ == "__main__":
    model_id = 0
    model_path = config.config["save_model_path"] + "/model_" + str(model_id) + ".pth"
